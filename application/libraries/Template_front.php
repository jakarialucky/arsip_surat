...
class Template_front
{
  ...
  public function  view($view_name, $params = array(), $default = true)
  {
    ...
    if ($default) {
      ...
      $header_params['main_menu'] = $this->CI->load->view("core_front/menu", [], true);
      $this->CI->load->view('core_front/header', $header_params);
      // $this->CI->load->view(config_item("cpanel").'sidebar',$header_params);
      // if (profile("id_group") != 1) {
      //   if (setting("maintenance_status") == "Y") {
      //     $this->CI->load->view("core_front/maintenance");
      //   } else {
      ...
      $this->CI->load->view($view_name, $params);
      // }
      $this->CI->load->view('core_front/footer');
    } else {
      $this->CI->load->view($view_name, $params);
    }
    // }else {
