<?php $show_in_update = $this->mcrud_build->showInUpdate(); ?>
<div class="row">
...
<?php if (formType($field) == "number"): ?>
            <input type="number" class="form-control form-control-sm" placeholder="<?=label($field)?>" name="<?=$field?>" id="<?=$field?>" value="{php_open_tag_echo}$<?=$field?>{php_close_tag}">
<?php elseif(formType($field) == "text"): ?>
            <input type="text" class="form-control form-control-sm" placeholder="<?=label($field)?>" name="<?=$field?>" id="<?=$field?>" value="{php_open_tag_echo}$<?=$field?>{php_close_tag}">
<?php elseif(formType($field) == "textarea"): ?>
            <textarea class="form-control form-control-sm" placeholder="<?=label($field)?>" name="<?=$field?>" id="<?=$field?>" rows="3" cols="80">{php_open_tag_echo}$<?=$field?>{php_close_tag}</textarea>
<?php elseif(formType($field) == "upload_image"): ?>
            ...
            <div class="input-group col-xs-12">
              ...
              <input type="text" class="form-control form-control-sm file-upload-info file-name" data-id="<?=$field?>" placeholder="<?=label($field)?>" readonly name="<?=$field?>" value="{php_open_tag_echo}$<?=$field?>{php_close_tag}" />
...
<?php elseif(formType($field) == "text_editor"): ?>
            <textarea class="form-control text-editor" rows="3" data-original-label="<?=$field?>" name="<?=$field?>" id="<?=$field?>">{php_open_tag_echo}$<?=$field?>{php_close_tag}</textarea>
...
<?php elseif(formType($field) == "option_relation"): ?>
            ...
            --->
            {php_open_tag_echo}is_radio("<?=optionRelation($field, "relation_table")?>","<?=$field?>","<?=optionRelation($field, "relation_value")?>","<?=optionRelation($field, "relation_label")?>","$<?=$field?>");{php_close_tag}
<?php elseif(formType($field) == "select_relation"): ?>
            ...
            --->
            {php_open_tag_echo}is_select("<?=optionRelation($field, "relation_table")?>","<?=$field?>","<?=optionRelation($field, "relation_value")?>","<?=optionRelation($field, "relation_label")?>","$<?=$field?>");{php_close_tag}
<?php elseif(formType($field) == "date"): ?>
            <input type="date" class="form-control form-control-sm" placeholder="<?=label($field)?>" name="<?=$field?>" id="<?=$field?>" value="{php_open_tag_echo}$<?=$field?>{php_close_tag}">
<?php elseif(formType($field) == "time"): ?>
            <input type="time" class="form-control form-control-sm" placeholder="<?=label($field)?>" name="<?=$field?>" id="<?=$field?>" value="{php_open_tag_echo}$<?=$field?>{php_close_tag}">
<?php elseif(formType($field) == "datetime"): ?>
            <input type="datetime-local" class="form-control form-control-sm" placeholder="<?=label($field)?>" name="<?=$field?>" id="<?=$field?>" value="{php_open_tag_echo}$<?=$field?>{php_close_tag}">
<?php endif; ?>
<?php if (formType($field) != "timestamp"): ?>
          </div>
<?php endif; ?>
        <?php endforeach; ?>
          ...
          <div class="text-right">
            ...
            <button type="submit" id="submit"  class="btn btn-sm btn-primary">{php_open_tag_echo}cclang("update"){php_close_tag}</button>
          </div>
        </form>
      </div>
    </div>
