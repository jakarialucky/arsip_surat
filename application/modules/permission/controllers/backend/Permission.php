...
class Permission extends Backend
{
  ...
  function add()
  {
    ...
    $this->template->set_title(cclang("add") . " Izin");
    $data = array(
      'action' => url("permission/add_action"),
      'button' => "save",
  ...
  function add_action()
  {
    if ($this->input->is_ajax_request()) {
      ...
      if ($this->form_validation->run()) {
        $save_data = [
          "permission"      =>  str_replace("-", "_", strtolower($this->input->post("permission", true))),
  ...
  function update($id = null)
  {
    ...
    if ($row = $this->model->find(dec_url($id))) {
      $this->template->set_title(cclang("update") . " Izin");
      $data = array(
        'action' => url("permission/update_action/$id"),
        'button' => "update",
