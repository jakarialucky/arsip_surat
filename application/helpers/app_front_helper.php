...
if (!function_exists('is_select')) {
	function is_select($table, $attribute, $value, $label, $entry_value = "", $disabled = "", $arr_ruangan = "")
	{
		$ci = &get_instance();
		$str = "";
		if ($arr_ruangan != "") {
			$ci->db->where_in("kode_ruangan", explode(",", $arr_ruangan));
			$qry = $ci->db->get($table);
		} else {
			$qry = $ci->db->get($table);
		}
		$str .= '<select class="form-control form-control-sm select2" data-placeholder=" -- Select -- " name="' . $attribute . '"' . $disabled . '>';
...
if (!function_exists('is_select_multiple')) {
	function is_select_multiple($table, $attribute, $value, $label, $entry_value = "", $disabled = "")
	{
		$ci = &get_instance();
		$str = "";
		$qry =  $ci->db->get($table);
		$str .= '<select class="form-control form-control-sm select2" data-placeholder=" -- Select -- " name="' . $attribute . '" ' . $disabled . ' multiple="multiple">';
		$str .= '<option value=""></option>';
		$entry_value = explode(",", $entry_value);
		foreach ($qry->result() as $row) {
			$str .= '<option ' . (array_search($row->$value, $entry_value) !== false  ? "selected" : "") . ' value="' . $row->$value . '">' . $row->$label . '</option>';
		}
		$str .= '</select>';
		$str .= '<div  id="' . $attribute . '"></div>';
...
if (!function_exists('date_fc')) {
	function date_fc($day = '')
	{
		$date = date("Y-m-d", strtotime($day . ' this week', strtotime(date("Y-m-d"))));
